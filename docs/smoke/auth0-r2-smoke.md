...
1. Bootstrap the local Auth0 smoke tenant, user, and membership from explicit safe values:
...
5. Click the login CTA and complete Auth0 login.

6. Confirm the app redirects back to the dashboard.

7. Confirm the protected dashboard loads user/tenant bootstrap data.

8. Confirm the frontend bridge endpoint works while authenticated:
...
10. Inspect local logs and response bodies. They may include safe diagnostics such as target URL, token format, `alg`, `kid`, `iss`, and `aud`; they must not include raw access tokens, refresh tokens, client secrets, database URLs, or full environment dumps.

11. Log out and confirm protected dashboard access requires authentication again.
